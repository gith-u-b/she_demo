...
function addTabed(){
		if($("#tab").tabs("exists","文章管理")){
			
			$("#tab").tabs("select","文章管理");
			
		}else{
			$('#tab').tabs('add',{   
			    title:"文章管理",   
			    content:"<iframe src='admin/articles/articles_list.jsp' width='99%' height='99%' scrolling='no' frameborder='0' ></iframe>",   
			    closable:true,
			    selected:true
...
function addTabed8(){
	if($("#tab").tabs("exists","密码修改")){
		...
		$("#tab").tabs('add',{   
		    title:"密码修改",   
		    content:"<iframe src='admin/user/personinfo.jsp' width='99%' height='99%' scrolling='no' frameborder='0' ></iframe>",   
		    closable:true,
		    selected:true
...
<!-- 服务器端脚本，强制浏览器重新访问服务器下载页面，而不从缓存读取，让它从新访问 -->
	...
	<div class="top_nav">
			<ul>
				<li><a href="<%=path %>/admin/page/main.jsp" class="selected">后台首页</a></li>
				<li><a href="javascript:addTabed()">文章管理</a></li>
			</ul>
	</div>
	...
	欢迎您，<font color="blue">${user.userName }</font> | <a href="javascript:addTabed8()">密码修改</a> 
	</div>
</div>
<div class="left" style="height: 550px">
	<div  class="member_info"><div class="member_ico">
		<img src="<%=path %>/admin/images/miao2.jpg" width="43" height="43"></div><a class="system_a" href="javascript:addTabed1()">系统设置</a><a href="<%=path %>/user_logout.action" class="system_log">注销</a> <a href="<%=path %>/user!logout.action" class="system_logout">退出</a>
	</div>
